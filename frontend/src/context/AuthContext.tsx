...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [user, setUser] = useState<User | null>(() => {
    const savedUser = localStorage.getItem('tradewatch_user');
    return savedUser ? JSON.parse(savedUser) : null;
  });
  const [token, setToken] = useState<string | null>(() => {
    return localStorage.getItem('tradewatch_token');
  });
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    const initAuth = async () => {
      const savedToken = localStorage.getItem('tradewatch_token');
      if (savedToken) {
        try {
          const userData = await getMeApi();
          setUser(userData);
          localStorage.setItem('tradewatch_user', JSON.stringify(userData));
        } catch (err) {
  ...
  const login = async (data: LoginData) => {
    ...
    setUser(res.user);
    localStorage.setItem('tradewatch_token', res.access_token);
    localStorage.setItem('tradewatch_user', JSON.stringify(res.user));
  ...
  const updateProfile = async (data: UserProfileUpdateData) => {
    ...
    setUser(updatedUser);
    localStorage.setItem('tradewatch_user', JSON.stringify(updatedUser));
